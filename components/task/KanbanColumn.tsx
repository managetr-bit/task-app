...
export function KanbanColumn({
  ...
  onAddTask, onAssignTask, onTaskClick, onDeleteColumn,
}: Props) {
  ...
  // Make the column a drop target (for cross-column drops onto empty columns)
    ...
    <div style={{ flexShrink: 0, width: 280, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 0.25rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: '#1a1a1a', letterSpacing: '-0.01em' }}>
            {column.name}
          </span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
          <button
            onClick={onAddTask}
            title="Add task"
            style={{ background: 'transparent', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: '1.1rem', lineHeight: 1, padding: '0.2rem 0.3rem', borderRadius: '6px', transition: 'color 0.15s ease' }}
            ...
            <>
              {showDeleteConfirm ? (
                <div style={{ display: 'flex', gap: '0.2rem' }}>
                  <button onClick={() => { onDeleteColumn(); setShowDeleteConfirm(false) }} style={{ fontSize: '0.65rem', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', padding: '0.1rem 0.25rem' }}>✓</button>
                  <button onClick={() => setShowDeleteConfirm(false)} style={{ fontSize: '0.65rem', color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', padding: '0.1rem 0.25rem' }}>✕</button>
                </div>
              ) : (
                <button
                  onClick={() => setShowDeleteConfirm(true)}
                  style={{ background: 'transparent', border: 'none', color: '#c4bfb9', cursor: 'pointer', fontSize: '0.75rem', padding: '0.2rem 0.3rem', borderRadius: '6px', transition: 'color 0.15s ease' }}
      ...
      >
        ...
        {hiddenCount > 0 && (
          <button
            ...
            style={{ background: 'transparent', border: 'none', borderRadius: '8px', padding: '0.4rem 0.5rem', cursor: 'pointer', color: '#9ca3af', fontSize: '0.75rem', fontWeight: 500, textAlign: 'center', transition: 'color 0.15s ease' }}
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.color = '#c9a96e' }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = '#9ca3af' }}
          >
        ...
        {!isDoneColumn && (
          <button
            onClick={onAddTask}
            style={{ background: 'transparent', border: '1.5px dashed #E8E5E0', borderRadius: '10px', padding: '0.5rem', cursor: 'pointer', color: '#c4bfb9', fontSize: '0.75rem', transition: 'border-color 0.15s ease, color 0.15s ease', textAlign: 'center' }}
