...
export default function LandingPage() {
  ...
  async function createBoard(p: Profile) {
    ...
    const path = activeVersion === 'v5' ? `/v5/${board.id}` : activeVersion === 'v4' ? `/v4/${board.id}` : activeVersion === 'v3' ? `/v3/${board.id}` : activeVersion === 'v2' ? `/v2/${board.id}` : `/${board.id}`
    router.push(path)
  }
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '0.375rem', background: '#EEEBF8', borderRadius: 10, padding: '0.25rem', marginBottom: '1.5rem' }}>
            {([
              ...
              { v: 'v5', label: 'v5 — Architect' },
            ] as { v: 'v1' | 'v2' | 'v3' | 'v4' | 'v5'; label: string }[]).map(({ v, label }) => (
              <button
                key={v}
                ...
                {recentBoards.slice(0, 6).map((rb, i) => (
                  ...
                  >
                    ...
                    <div style={{ padding: '0.875rem 1rem' }}>
                      <div
                        onClick={() => router.push(activeVersion === 'v5' ? `/v5/${rb.boardId}` : activeVersion === 'v4' ? `/v4/${rb.boardId}` : activeVersion === 'v3' ? `/v3/${rb.boardId}` : activeVersion === 'v2' ? `/v2/${rb.boardId}` : `/${rb.boardId}`)}
